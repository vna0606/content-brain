...
async def cmd_ideas(message: Message):
    ...
    conn = _get_db()
    cur = conn.execute(
        "SELECT id, title, thesis FROM cb_ideas WHERE status = 'new' ORDER BY created_at DESC LIMIT 10"
    )
    rows = cur.fetchall()
    conn.close()
    ...
    for i, row in enumerate(rows, 1):
        idea_id, title, thesis = row[0], row[1], row[2]
        short = thesis[:100] + "..." if len(thesis) > 100 else thesis
        text_lines.append(f"<b>{i}. {title}</b>\n{short}\n")
        buttons.append([
            InlineKeyboardButton(text=f"{i}. {title[:40]}", callback_data=f"idea:{idea_id}")
        ])
...
async def cb_select_idea(callback: CallbackQuery):
    ...
    conn.execute("UPDATE cb_ideas SET status = 'shown' WHERE id = ?", (idea_id,))
    conn.commit()
    conn.sync()
    conn.close()

    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="Написать пост", callback_data=f"write:{idea_id}")],
